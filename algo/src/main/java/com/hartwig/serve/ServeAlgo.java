...
public class ServeAlgo {
    ...
    public Map<RefGenome, ExtractionResult> run(@NotNull ServeConfig config) throws IOException {
        Stream<ExtractionResult> ckbExtractions = (config.useCkbEvidence() || config.useCkbTrials()) ?
                extractCkbKnowledge(config) : Stream.empty();
                
        List<ExtractionResult> extractions = Stream.concat(ckbExtractions, Stream.of(
            ...
            config.useHartwigCohortHotspots() ?
                    extractHartwigCohortHotspotKnowledge(config.hartwigCohortHotspotTsv(), !config.skipHotspotResolving()) : null,
            config.useHartwigCuratedHotspots() ?
                    extractHartwigCuratedHotspotKnowledge(config.hartwigCuratedHotspotTsv(), !config.skipHotspotResolving()) : null,
    ...
    private Stream<ExtractionResult> extractCkbKnowledge(ServeConfig config) throws IOException {
        List<CkbEntry> ckbEntries = CkbReader.readAndCurate(config.ckbDir(), config.ckbFilterTsv());
        
        return Stream.of(
                config.useCkbEvidence() ?
                        extractCkbEvidenceKnowledge(config.ckbDrugCurationTsv(), ckbEntries) : null,
                config.useCkbTrials() ? extractCkbTrialKnowledge(ckbEntries) : null
        );
    }

    @NotNull
    private ExtractionResult extractCkbEvidenceKnowledge(@NotNull String ckbDrugCurationTsv, List<CkbEntry> ckbEntries) throws IOException {
    ...
    @NotNull
    private ExtractionResult extractCkbTrialKnowledge(List<CkbEntry> ckbEntries) {
        EventClassifierConfig config = CkbClassificationConfig.build();
