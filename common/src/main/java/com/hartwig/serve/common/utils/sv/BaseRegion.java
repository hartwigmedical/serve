...
public class BaseRegion implements Cloneable, Comparable<BaseRegion> {
    ...
    public int start() {
        return Positions[SE_START];
    }

    public int end() {
        return Positions[SE_END];
    }
    ...
    public void setStart(int pos) {
        setPosition(pos, SE_START);
    }

    public void setEnd(int pos) {
        setPosition(pos, SE_END);
    }
    ...
    public int length() {
        return Positions[SE_END] - Positions[SE_START];
    }

    public boolean hasValidPositions() {
        return Positions[SE_START] > 0 & Positions[SE_END] >= Positions[SE_START];
    }
    ...
    public String toString() {
        return String.format("%d-%d", Positions[SE_START], Positions[SE_END]);
    }
